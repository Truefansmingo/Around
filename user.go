...
func checkUser(username, password string) (bool, error) {
	...
	for _, item := range searchResult.Each(reflect.TypeOf(utype)) {
		if u, ok := item.(User); ok {
			// check password, if correct, print login in
...
func handlerLogin(w http.ResponseWriter, r *http.Request) {
	...
	// generate new token claim is key, value pair in json
	// create a new token obj, specifying signing method and the claims
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		"username": user.Username,
		"exp":      time.Now().Add(time.Hour * 24).Unix(),
	})
	//Sign and get tge complete encoded token as a string using the secret
